...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Step 11.1.4 base-cost versus zero-cost trend ablation.")
...
def main() -> int:
    ...
    base_result, zero_result = run_step11_cost_ablation(
        prices=prices, benchmark_returns=benchmark, risk_free_returns=risk_free
    )
    paths = export_step11_cost_ablation(
        base_result, zero_result, args.output_dir, benchmark_path=args.benchmark_path
    )
    base = pd.to_numeric(base_result.fold_summary["net_excess_nav_difference"], errors="coerce").dropna()
    zero = pd.to_numeric(zero_result.fold_summary["net_excess_nav_difference"], errors="coerce").dropna()
    comparison = pd.read_csv(paths["comparison"])
    print("step=11.1.4")
    print("analysis_role=diagnostic_ablation")
    print(f"folds={len(base)}")
    print(f"base_mean_net_excess_nav_difference={base.mean():.12f}")
    print(f"zero_mean_net_excess_nav_difference={zero.mean():.12f}")
    print(f"mean_cost_effect_nav_difference={(zero.mean() - base.mean()):.12f}")
